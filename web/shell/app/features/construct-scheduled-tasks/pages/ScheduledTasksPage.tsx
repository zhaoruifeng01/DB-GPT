...
} from '~/features/construct-scheduled-tasks/queries';
...
export default function ScheduledTasksPage() {
  ...
  const { data: tasks = [], isFetching: loading, refetch } = useScheduledTasks();
  ...
  const onDelete = async (task: TaskResponse) => {
    try {
      ...
      message.error(err instanceof Error ? err.message : t('scheduled.msg.deleteFailed'));
    }
  };

  const onEdit = (task: TaskResponse, e?: MouseEvent) => {
    e?.stopPropagation();
    setEditingTask(task);
    setEditOpen(true);
  };
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter(task => {
      ...
      return (
        ...
        task.cron_expression.toLowerCase().includes(q)
      );
    });
    ...
    <ConstructLayout className='scrollable-tabs'>
      <div className='relative w-full bg-gradient-to-b from-[#f7f8fc] via-white to-[#f7f8fc] dark:from-[#1c2333] dark:via-[#1c2333] dark:to-[#161b29]'>
        <div className='max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8'>
          ...
          <Spin spinning={loading || toggleTask.isPending || deleteTask.isPending}>
            {!hasAny && !loading ? (
              ...
              <div className='rounded-2xl border border-white/80 bg-white/80 backdrop-blur-lg shadow-[0_2px_12px_-4px_rgba(15,23,42,0.08)] dark:border-[#3a4456] dark:bg-[#2b303d]/70 overflow-hidden'>
                ...
                {filteredTasks.map(task => (
                  <TaskRow
                    ...
                    onEdit={onEdit}
                    onDelete={onDelete}
                    onClick={() => navigate(`/construct/scheduled-tasks/${task.task_id}`)}
                  />
                ))}
...
interface TaskRowProps {
  ...
  onEdit: (task: TaskResponse, e?: MouseEvent) => void;
  onDelete: (task: TaskResponse) => void;
  onClick: () => void;
}

function TaskRow({ task, onToggle, onEdit, onDelete, onClick }: TaskRowProps) {
  const { t } = useTranslation();
