...
import { useConnectorList } from '~/features/construct-connectors/queries';
import EditScheduledTaskDrawer from '~/features/construct-scheduled-tasks/components/EditScheduledTaskDrawer';
import TaskRunsTable from '~/features/construct-scheduled-tasks/components/TaskRunsTable';
import { useScheduledTaskDetail, useToggleScheduledTask } from '~/features/construct-scheduled-tasks/queries';
...
export default function ScheduledTaskDetailPage() {
  ...
  const { data: task, isFetching: loading, refetch } = useScheduledTaskDetail(taskId);
  ...
  const connectorNameMap = useMemo(() => {
    ...
    for (const connector of connectors) {
      map.set(connector.id, connector.display_name);
    }
    return map;
  }, [connectors]);
  ...
  const onToggle = async (enabled: boolean) => {
    if (!task) return;
    try {
      await toggleTask.mutateAsync({ taskId: task.task_id, enabled });
      message.success(enabled ? t('scheduled.msg.enabled') : t('scheduled.msg.paused'));
    } catch (err) {
      message.error(err instanceof Error ? err.message : t('scheduled.msg.opFailed'));
    }
  };
  ...
  const ext = (task.payload?.ext_info ?? {}) as Record<string, unknown>;
  const connectorIds = [...stringArray(ext.connector_ids), ...stringArray(ext.mcp_ids)];
    ...
    <ConstructLayout className='scrollable-tabs'>
      <div className='relative w-full bg-gradient-to-b from-[#f7f8fc] via-white to-[#f7f8fc] dark:from-[#1c2333] dark:via-[#1c2333] dark:to-[#161b29]'>
        <div className='max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8'>
          <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/construct/scheduled-tasks')} className='mb-5'>
              ...
              <div className='flex items-start gap-4'>
                <div
                  className={`flex-shrink-0 w-14 h-14 rounded-xl flex items-center justify-center text-white text-2xl shadow-[0_4px_14px_-4px_rgba(6,182,212,0.5)] ${
                    task.enabled ? 'bg-gradient-to-br from-blue-500 to-cyan-600' : 'bg-gradient-to-br from-gray-400 to-gray-500'
                  }`}
                >
                  <ClockCircleOutlined />
                </div>
          ...
          <div className='rounded-2xl border border-white/80 bg-white/80 backdrop-blur-lg shadow-[0_2px_12px_-4px_rgba(15,23,42,0.08)] dark:border-[#3a4456] dark:bg-[#2b303d]/70 p-6 mb-5'>
            ...
            <div className='grid grid-cols-1 gap-y-4'>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4'>
                ...
                {Boolean(ext.knowledge_space_name) && (
...
function InfoField({ label, className, children }: InfoFieldProps) {
  return (
