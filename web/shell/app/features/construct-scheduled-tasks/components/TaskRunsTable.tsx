...
import { useTaskRuns } from '~/features/construct-scheduled-tasks/queries';
...
export default function TaskRunsTable({ taskId }: TaskRunsTableProps) {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const { data: runs = [], isFetching: loading, refetch } = useTaskRuns(taskId, 50, 0);
  ...
  const handleView = useCallback(
    (run: RunResponse) => {
      ...
      navigate(`/?id=${run.output_conv_uid}&from_task=${taskId}`);
    },
    [navigate, taskId, t],
  );
    ...
    <div>
      ...
      <div className='overflow-hidden rounded-xl border border-gray-100 dark:border-gray-700/50'>
        ...
        {runs.map(run => {
          ...
          const summaryText = isFailed ? (run.error_message ?? '').slice(0, 80) : (run.result_summary ?? '').slice(0, 80);
          const duration =
