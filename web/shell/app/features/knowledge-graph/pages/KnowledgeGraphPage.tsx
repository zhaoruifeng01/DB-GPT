...
import { useKnowledgeGraph } from '~/features/knowledge-graph/queries';

const LIMIT = 500;
const PALETTE = ['#5F95FF', '#61DDAA', '#F6BD16', '#7262FD', '#78D3F8', '#9661BC', '#F6903D', '#008685', '#F08BB4'];

type GraphVisData = GraphVisResult | null | undefined;

function transformData(data: GraphVisData): GraphData {
  if (!data) return { nodes: [], edges: [] };

  const nodes = data.nodes.map(node => ({ id: String(node.id), data: node }));
...
export default function KnowledgeGraphPage() {
  ...
  const graphQuery = useKnowledgeGraph(spaceName, LIMIT);
  const graphData = useMemo(() => transformData(graphQuery.data), [graphQuery.data]);

  useEffect(() => {
    if (!isReady || !graphRef.current) return;
  ...
  const getNodeSize = (nodeId: ID) => getSize(getNodeDegree(nodeId));
  ...
  if (!spaceName) {
    return (
      <div className='flex h-full items-center justify-center'>
        <Empty description='Missing spaceName' />
      </div>
    );
  }

  if (graphQuery.isFetching) {
    return <Spin className='h-full content-center justify-center' />;
  }

  return (
    <div className='relative h-full overflow-y-scroll p-4 px-2'>
      <Graphin
        ref={ref => {
      ...
      >
        <Button style={{ background: '#fff' }} onClick={() => navigate('/construct/knowledge')} icon={<RollbackOutlined />}>
          Back
        </Button>
      </Graphin>
    </div>
