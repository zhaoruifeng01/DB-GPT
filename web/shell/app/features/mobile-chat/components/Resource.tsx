...
const Resource: React.FC = () => {
  ...
  const temperatureValue = appInfo?.param_need?.find(item => item.type === 'temperature')?.value ?? 0.6;
  const maxNewTokensValue = appInfo?.param_need?.find(item => item.type === 'max_new_tokens')?.value ?? 4000;
