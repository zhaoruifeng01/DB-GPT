...
const MobileChat: React.FC = () => {
  ...
  const { run: getDialogueListRun, loading: dialogueListLoading } = useRequest(
    ...
    },
    {
      ...
      onSuccess: data => {
        ...
        if (filterDialogue?.select_param) {
          setResource(JSON.parse(filterDialogue.select_param));
        }
      },
    },
  );
  ...
  useEffect(() => {
    if (chatScene && appInfo?.app_code) {
      ...
      if (bindResourceVal) {
        setResource(bindResourceVal);
      }
      if (['database', 'knowledge', 'plugin', 'awel_flow'].includes(resourceVal) && !bindResourceVal) {
        run();
      }
    }
