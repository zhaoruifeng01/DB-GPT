...
export const deleteUser = (userId: number) => DELETE<null, any>(`${API_PREFIX}/users/${userId}`);

/** 角色管理 */
export const listRoles = () => GET<null, RoleResponse[]>(`${API_PREFIX}/roles`);

export const createRole = (data: RoleRequest) => POST<RoleRequest, RoleResponse>(`${API_PREFIX}/roles`, data);

export const updateRole = (roleId: number, data: RoleRequest) =>
  PUT<RoleRequest, RoleResponse>(`${API_PREFIX}/roles/${roleId}`, data);

export const deleteRole = (roleId: number) => DELETE<null, any>(`${API_PREFIX}/roles/${roleId}`);

/** 部门管理 */
export const listDepts = () => GET<null, DeptResponse[]>(`${API_PREFIX}/depts`);

export const createDept = (data: DeptRequest) => POST<DeptRequest, DeptResponse>(`${API_PREFIX}/depts`, data);

export const updateDept = (deptId: number, data: DeptRequest) =>
  PUT<DeptRequest, DeptResponse>(`${API_PREFIX}/depts/${deptId}`, data);

export const deleteDept = (deptId: number) => DELETE<null, any>(`${API_PREFIX}/depts/${deptId}`);
